...
def model_fitting_SM(model_type,n_iter:float=1e5,mutant_range:slice=slice(0,len(SM_names))):
...
#%%
def get_WT_params(model_type,start_guess:list,n_iter:float=1e5):
    #this function will estimate the wild type parameters for a given model.
    #now loading wt dataframe
    if start_guess==[]:
        start_guess=[1]*20
    data_=meta_dict['WT']
    min_result=minimize(min_fun,args=(data_,model_type),x0=start_guess,method='Nelder-Mead',bounds=generate_bounds(),options={"maxiter":n_iter,"disp":True})
...
def generate_bounds(params_dict:dict,node:str,custom_settings:list=[]):
    ...
    if node=="Sensor":
        ...
        for i in range(0,len(keys_indices_to_change)):
            ...
            bnds_list[keys_indices_to_change[i]]=[keys_values_to_keep_constant[i],keys_values_to_keep_constant[i]]
    elif node=="Regulator":
    ...
    list(params_dict['free_params'].keys())
    keys_indices_to_change== [i for i,x in enumerate(param_names) if x in keys]
    for i in range(0,len(keys_indices_to_change)):
