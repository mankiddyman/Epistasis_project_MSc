...
class model_hill_shaky:
    def __init__(self,params_list:list,I_conc):
        self.params_list=params_list
        self.I_conc=I_conc
        self.example_dict={"sen_params":{"A_s":600,"B_s":15000,"C_s":1,"N_s":1},"reg_params":{"A_r":1,"B_r":1,"C_r":1,"N_r":1},"out_h_params":{"A_h":1,"B_h":1,"C_h":1},"out_params":{"A_o":1,"B_o":1,"C_o":1,"N_o":1},"free_params":{"F_o":1}}
    ...
    def model(params_list:list,I_conc):
        ...
        O0 = (A_o + B_o/(1+np.power(C_h*(S0+C_o*R0),N_o)))*F_o
        ...
        for conc in I_conc:
            ...
            R = odeint(ODE_R, R0, t, args = (float(S),))
            np.append(Regulator, R)
            H = odeint(ODE_O, H0, t, args = (S,))[-1]
            Regulator = np.append(Output_half, H)
...
def competition_model(params_list:list,I_conc, compDeg = False):
...
@staticmethod    
def model_3(params_list,I_conc, compDeg=False): #reformulated so that output half and output are same parameters with F_o to scale
    if compDeg == False:
        correct_length=13
    else:
        correct_length=15
    ...
    ##############################################################
    if compDeg == True:
        K= params_list[13] #affinity for S, R & O for protease
        Deg = params_list[14] #average linear degredation term for S, R & O
    #S (Sensor) PRODUCED is constant for a given I conc
    S_prod = A_s+B_s*np.power(C_s*I_conc,N_s)
    S_prod /= 1+np.power(C_s*I_conc,N_s)

    #Amount of R (Regulator) PRODUCED depends only on the amount of Sensor at a given time
    def R_prod(S):
        R_prod = B_r/(1+np.power(C_r*S,N_r))
        R_prod += A_r
    ...
    def O_SRO(S):
        ...
        c = 1+K(S+R)
        return max((-b+np.power(b**2-4*a*c))/2*a, (-b-np.power(b**2-4*a*c))/2*a)
    ...
    def F_SRO(S):
        R = R_SRO(S)
        O = O_SRO(S)
        return S_prod - S/(1+S+R+O) - Deg*S
    
    Sensor = S_S
    Regulator = R_SR = 
    Out_half = O_SO = 
    S__SRO = root_scalar(F_SRO , x0 = 0, x1 = 2000, method='secant', xtol=1e-3)
    Output = O_SRO = 
